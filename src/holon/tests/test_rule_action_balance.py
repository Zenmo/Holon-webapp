...
class RuleMappingTestClass(TestCase):
    def setUp(self) -> None:
        ...
        self.balance_group: RuleActionBalanceGroup = RuleActionBalanceGroup.objects.create(
            rule=rule
        )
    ...
    def count_models(self, model_list: list, model_type_name: str) -> int:
        return len([model for model in model_list if model.__class__.__name__ == model_type_name])
