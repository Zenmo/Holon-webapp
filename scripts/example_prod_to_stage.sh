...
readonly STAGE_HOST=deploy@stage.holontool.nl
readonly PROD_HOST=deploy@holontool.nl
...
ssh $PROD_HOST "pg_dump -h localhost -Fc -f /tmp/db-dump.sql -U postgres holon_wagtail -x -O"
...
ssh $STAGE_HOST "sudo -u postgres pg_restore --clean -h localhost -d holon_wagtail -U postgres '/tmp/db-dump.sql'"
ssh $STAGE_HOST "rm /tmp/db-dump.sql"

rm /tmp/db-dump.sql

echo "Updating database..."
manage_prefix="source /mnt/persist/www/holon_wagtail/shared/venv/bin/activate && cd /mnt/persist/www/holon_wagtail/current/src &&"

ssh $STAGE_HOST "$manage_prefix python manage.py change_site_domain --site_id=1 --new_site_domain='stage.holontool.nl'"

echo "Syncing s3 buckets..."
aws --profile holon_wagtail_devops s3 sync s3://s3.stage.holontool.nl s3://s3.holontool.nl --acl public-read
