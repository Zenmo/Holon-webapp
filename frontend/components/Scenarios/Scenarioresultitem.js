...
function Scenarioresultitem({ minvalue, maxvalue, label, unit, value, local, invert, messageLocal, messageNl }) {
                ...
                {inputvalue >= 0 &&
                    <span className="relative ml-auto">
                        <Tooltip tooltipMessage={local ? messageLocal : messageNl}>
                            <output style={{ backgroundColor: perc2color(minvalue, maxvalue, inputvalue, invert) }} className={`border rounded-full block text-center leading-[3rem] h-[3rem] w-[3rem] shadow-[2px_2px_0_0]`}>{inputvalue}</output>
